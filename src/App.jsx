...
export default function App() {
  ...
  const callAi = async (prompt, systemPrompt = "You are a helpful literary assistant.") => {
    ...
    try {
      ...
      while (true) {
        ...
        lines.forEach(line => {
          ...
          try {
            ...
            finalAnswer = answer; // Keep track of latest answer for return value
      ...
      return finalAnswer; // Return for Insights/Translation handlers
    } catch (err) {
  ...
  const handleInsight = async (type) => {
    ...
    // Insights now return text for the results box
    const result = await callAi(p, s);
  ...
  const handleChat = async () => {
    ...
    if (social.some(s => lowerQ.startsWith(s))) {
      const reply = lowerQ.includes('thank') ? "You're very welcome! Happy to help." : "Hello! I'm your literary guide. Ask me anything about this manuscript!";
  ...
  const handleTranslate = async () => {
    ...
    const res = await callAi(
        ...
        `You are a professional literary translator. Provide ONLY the translation.`
    );
    // Translation result logic is handled by callAi updating chat, 
    // but we ensure we are in the chat tab to see it.
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <nav className="hidden md:flex w-20 border-r border-zinc-200 dark:border-zinc-800 flex-col items-center py-8 gap-8 bg-white dark:bg-zinc-900 z-50">
          ...
          <div className="mt-auto flex flex-col gap-4">
            <button onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')} className="p-3 text-zinc-400 hover:text-amber-500 transition-colors">
              {theme === 'dark' ? <Sun size={20}/> : <Moon size={20}/>}
            </button>
            <button onClick={hardReset} className="p-3 text-zinc-400 hover:text-red-500 transition-colors">
              <RefreshCw size={20}/>
            </button>
          </div>
        </nav>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-20 py-8 scroll-smooth relative custom-scrollbar bg-zinc-50 dark:bg-zinc-950">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-12 pb-32">
              {pages.map((p, i) => (
                <article key={i} id={`page-${i}`} data-page-index={i}
